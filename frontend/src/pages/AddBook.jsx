...
const AddBook = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      showToast('Failed to load category definitions.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      showToast(parseApiError(error, 'Failed to save book record.'), 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-50">Add New Book</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Create a new book entry in the library archive catalog.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          <div className="glass-card p-6 md:p-8 space-y-6">
            ...
            <Input
              ...
              onChange={handleInputChange}
              placeholder="Provide a brief summary of the book content, topics, target audience..."
